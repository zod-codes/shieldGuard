...
const TrackingDisplay = ({ trackingNumber, isActive }: TrackingDisplayProps) => {
    ...
    const { activeStages, currentStatus, isInterrupted, resumeTracking } = useLiveTracking(journeyStages, isTrackingActiveRef);

    const lastStage = activeStages.length > 0 ? activeStages[activeStages.length - 1] : null;

    // Start the tracking again
    const unlocked = false; // Change this to true to resume
